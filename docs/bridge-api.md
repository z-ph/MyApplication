...
| `id` | string | 配置 ID |
| `name` | string | 显示名 |
| `provider` | string | Provider 枚举字符串（如 `OPENAI`、`ZHIPU` 等） |
...
以下为方法返回包装的约定形状，实现时保持字段名：

| 场景 | 形状 |
|------|------|
| 会话列表 | `{ "sessions": ChatSessionDto[] }` |
| 单会话 | `{ "session": ChatSessionDto }` |
| 消息列表 | `{ "messages": ChatMessageDto[] }` |
| 配置列表 | `{ "configs": ApiConfigDto[] }` 或直接数组（实现选定后固定并更新本文） |
| Agent 状态 | `AgentStateDto` 或 `{ "state": AgentStateDto }`（实现选定后固定） |
| 权限状态 | 见 `LingxiPermission.getStatus`（字段在实现插件时补充并回写本文） |
| 版本 | `{ "version": string, ... }` |

**变更规则：** 新增可选字段可向后兼容；重命名 / 删除 / 改语义必须升文档版本说明并同步 TS 与 Facade。
...
| 方法 | 示意参数 | 示意返回 | 说明 |
|------|----------|----------|------|
| `listSessions` | — | `{ sessions: ChatSessionDto[] }` | 会话列表 |
...
| `listMessages` | `{ sessionId: string }` | `{ messages: ChatMessageDto[] }` | 消息列表 |
| `sendMessage` | `{ content: string }` | `void` | 发送用户指令并启动任务 |
| `cancelTask` | — | `void` | 取消进行中任务 |
| `clearMessages` | — | `void` | 清空当前会话消息（语义以 Facade 为准） |

| 事件 | 载荷（约定） | 说明 |
|------|--------------|------|
| `sessionsChanged` | `{ sessions: ChatSessionDto[] }` 或空（客户端 re-fetch） | 会话集合变化 |
| `messagesChanged` | `{ sessionId: string, messages?: ChatMessageDto[] }` | 消息变化 |
| `taskProgress` | 进度摘要 / 与 Agent 态相关 payload | 任务进度推送 |

### 3.2 `LingxiAgent`

| 方法 | 说明 |
|------|------|
| `getState` | 当前 `AgentStateDto` |
| `reconfigure` | 按活跃 API 配置重建 / 重载 Agent |
| `isConfigured` | 是否已具备可运行配置 |

| 事件 | 说明 |
|------|------|
| `stateChanged` | Agent 状态变更（建议原生 throttle 100–200ms） |

### 3.3 `LingxiApiConfig`

| 方法 | 说明 |
|------|------|
| `list` | 配置列表（含 `apiKeyMasked`，无明文 key） |
| `create` | 新建（请求含完整 `apiKey`） |
| `update` | 更新（可含完整 `apiKey`） |
| `delete` | 删除 |
| `setActive` | 设为活跃配置并触发 reconfigure |
| `fetchModels` | 拉取可用模型列表 |
| `testConnection` | 测连 |

| 事件 | 说明 |
|------|------|
| `configsChanged` | 配置列表或活跃项变化 |
